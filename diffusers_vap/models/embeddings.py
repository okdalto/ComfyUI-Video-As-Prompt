...
def get_3d_rotary_pos_embed(
    ...
    if grid_type == "linspace":
        ...
        # print(f"Original grid_t: {grid_t}")
        # print(f"Original grid_w: {grid_w}")
        # print(f"Original grid_h: {grid_h}")
        if mot_num > 0:
            if ref_type == "continous_negative":
                orig_t_start = 0
                orig_t_stop = temporal_size * (temporal_size - 1) / temporal_size
                # orig_w_start = start[1]
                # orig_w_stop = stop[1] * (grid_size_w - 1) / grid_size_w
                
                t_range = orig_t_stop - orig_t_start + 1
                # w_range = orig_w_stop - orig_w_start + 1
                ...
                # base_grid_t_offset = 50
                # grid_t = torch.linspace(base_grid_t_offset, base_grid_t_offset + mot_num * t_range-1, temporal_size, device=device, dtype=torch.float32)

                # grid_w = torch.linspace(-w_range, -1, grid_size_w, device=device, dtype=torch.float32)
                ...
                # print(f"discrete_long_reference, grid_t: {grid_t}")
            else:
        ...
        grid_t = torch.arange(temporal_size, device=device, dtype=torch.float32)
        if mot_num > 0:
            # grid_w = torch.arange(-mot_num * max_w, 0, device=device, dtype=torch.float32)
