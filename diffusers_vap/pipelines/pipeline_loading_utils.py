...
def _resolve_library_name(library_name: str) -> str:
    if library_name == "diffusers" or library_name.startswith("diffusers."):
        base = __name__.split(".")[0]
        suffix = library_name[len("diffusers") :]
...
def maybe_raise_or_warn(
    ...
    if not is_pipeline_module:
        library = importlib.import_module(_resolve_library_name(library_name))
...
def get_class_obj_and_candidates(
    ...
    resolved_library_name = _resolve_library_name(library_name)
    ...
    if is_pipeline_module:
        ...
        library = importlib.import_module(resolved_library_name)
